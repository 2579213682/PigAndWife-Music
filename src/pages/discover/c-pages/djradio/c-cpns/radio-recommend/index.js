...
import PigAndWifeRadioRecomendCover from "@/components/radio-recommend-cover";
import { RecommendWrapper } from "./style";

export default memo(function PigAndWifeRadioRecommend() {
  // redux
  ...
  return (
    <RecommendWrapper>
      <PigAndWifeThemeHeaderNormal title="优秀新电台" />
      <div className="radio-list">
        {" "}
        {recommends.slice(0, 5).map((item) => {
          return <PigAndWifeRadioRecomendCover info={item} key={item.id} />;
        })}{" "}
      </div>{" "}
    </RecommendWrapper>
  );
