...
import PigAndWifeSongOperationBar from "@/components/song-operation-bar";
import { InfoWrapper, InfoLeft, InfoRight } from "./style";

export default memo(function PigAndWifePlayerInfo() {
  // props and hooks
  ...
  return (
    <InfoWrapper>
      <InfoLeft>
        ...
        <div className="link">
          <i className="sprite_icon2"> </i> <a href="#/"> 生成外联播放器 </a>{" "}
        </div>{" "}
      </InfoLeft>{" "}
      <InfoRight isSpread={isSpread}>
        ...
        <div className="lyric">
          ...
          >
            {" "}
            {isSpread ? "收起" : "展开"} <i className="sprite_icon2"> </i>{" "}
          </button>{" "}
        </div>{" "}
      </InfoRight>{" "}
    </InfoWrapper>
